<h2>Medaille bearbeiten: <?= $name ?></h2>

<form action="?p=admin&s=edit-medal&id=<?= $id ?>&token=<?= $token ?>" method="post" enctype="multipart/form-data">
	<table class="form">
		<tr>
			<td><h3>Name:</h3></td>
				...
				<label>
					<select name="category">
						<?php foreach ($categories as $category): ?>
							<option
								value="<?= $category['id'] ?>" <?= ($categoryId === $category['id']) ? 'selected="selected"' : '' ?>><?= $category['name'] ?></option>
						<?php endforeach; ?>
					</select>
				</label>
			</td>
				...
				<label>
					<select name="award-condition">
						<?php foreach ($awardConditions as $name => $caption): ?>
							<option
								value="<?= $name ?>" <?= ($awardCondition === $name) ? 'selected="selected"' : '' ?>><?= $caption ?></option>
						<?php endforeach; ?>
					</select>
				</label>
			</td>
				...
				<label>
					<select name="image-filename">
						<?php foreach ($imageFilenames as $filename): ?>
							<option
								value="<?= $filename ?>" <?= ($filename === $imageFilename) ? 'selected="selected"' : '' ?>><?= $filename ?></option>
						<?php endforeach; ?>
					</select>
				</label>
	...
	<button class="primary" type="submit" name="submit">Medaille bearbeiten</button>
</form>
